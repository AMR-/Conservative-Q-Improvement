...
cdef extern from "../../include/qtreenode.hpp":
    cdef cppclass QTreeNode:
        ...
        vector[double]* getQVS(State*)
        void update(State*, Action*, int, unordered_map[string, double]*)
        void update(State*, Action*, int, unordered_map[string, double]*, int)
...
cdef extern from "../../include/qtree.hpp":
    cdef cppclass QTree:
        ...
        void update(State*, Action*, double, State*, bint, int)
        int numNodes()
        void printStructure()
        bint justSplit()
...
cdef class PyQTree:
    ...
    def __cinit__(self, PyBox state_space, PyDiscrete action_space, None, double \
        ...
        split_thresh_decay, int num_splits, int cql):
        self.thisptr = new QTree(state_space.thisptr, action_space.thisptr, NULL, \
        gamma, alpha, visit_decay, split_thresh_max, split_thresh_decay, num_splits, cql)
    def select_a(self, PyState s):
        return self.thisptr.selectA(s.thisptr)
    def select_a_with_cql(self, PyState s):
        return self.thisptr.selectAWithCQL(s.thisptr)
    ...
    def update(self, PyState s, PyAction a, double r, PyState s2, bint done, int index):
        return self.thisptr.update(s.thisptr, a.thisptr, r, s2.thisptr, done, index)
    def num_nodes(self):
