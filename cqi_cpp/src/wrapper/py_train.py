...
class Train(object):
    ...
    def train(self, num_steps, eps_func, verbose=False, eval_only=False, penalty_check=lambda s, r: 0,
        ...
        for step in range(num_steps):
            if done:
                if verbose:
                    print('Episode reward: ' + str(ep_r) +
                          '; Elapsed steps: ' + str(step))
                ...
                if track_data_per > 0 and num_eps % track_data_per == 0:
                    ...
                    print("At episode %d, hist is %d, %d" % (num_eps, num_nodes, ep_r))
            ...
            if not eval_only:
                ...
                if qfunc_hist is not None and self.qfunc.just_split():
                    # qfunc_hist.append(deepcopy(self.qfunc))
        ...
        if eval_only:
            ...
            results = "Num_eps: %d\nReward per ep: %f\nTimesteps per ep: %f\nPenalties per ep: %f" % (
                num_eps, avg_r_per_ep, np.mean(ts_per_ep), np.mean(pen_per_ep)
            )
            print(results)
