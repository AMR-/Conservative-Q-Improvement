...
LeafSplit::LeafSplit(int feature, double value, vector<double>* leftQS, 
    vector<double>* rightQS, vector<vector<double>*>* leftQVS, vector<vector<double>*>* rightQVS, 
    ...
    this->leftQVS = leftQVS;
    this->rightQVS = rightQVS;

    this->leftVisits = leftVisits;
    this->rightVisits = rightVisits;
}

void LeafSplit::update(State* s, Action* a, int target, unordered_map<string, double>* params) {
...
void LeafSplit::update(State* s, Action* a, int target, unordered_map<string, double>* params, int index) {
    double visitDecay = params->at("visitDecay");
    double alpha = params->at("alpha");
    vector<double>* leftQS = this->leftQVS->at(index);
    vector<double>* rightQS = this->rightQVS->at(index);
    
    this->leftVisits = this->leftVisits * visitDecay;
    this->rightVisits = this->rightVisits * visitDecay;

    if (s->state->at(this->feature) < this->value) {
        leftQS->at(a->value) = (1 - alpha) * leftQS->at(a->value) + alpha * target;
        this->leftVisits = this->leftVisits + (1 - visitDecay);
    } else {
        rightQS->at(a->value) = (1 - alpha) * rightQS->at(a->value) + alpha * target;
        this->rightVisits = this->rightVisits + (1 - visitDecay);
    }
}
...
double LeafSplit::evalUtility(vector<double>* polQVals, int index) {
    int actionChosen = Utils::argmax(polQVals);
    vector<double>* leftQS = this->leftQVS->at(index);
    vector<double>* rightQS = this->rightQVS->at(index);

    double leftQSMax = Utils::max(leftQS);
    double rightQSMax = Utils::max(rightQS);
    
    double leftPotUtil = leftQSMax - leftQS->at(actionChosen);
    double rightPotUtil = rightQSMax - rightQS->at(actionChosen);
